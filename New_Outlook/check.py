...
# Function to generate a daily serial number
def generate_serial_number():
    global current_date, daily_serial_number

    today = datetime.now().date()

    if current_date != today:
        current_date = today
        daily_serial_number = 1
    else:
        daily_serial_number += 1

    return daily_serial_number
def contains_job_description(email_body):
    jd_keywords = ['job description', 'jd', 'role responsibilities']
    email_body_lower = email_body.lower()
    return any(keyword in email_body_lower for keyword in jd_keywords)

def process_email(email):
    global daily_serial_number
    ...
    if sender in PROFILE_SENDERS and job_id in JOB_IDS:
        ...
        jd_status = "No" # Default value for JD status
        if html_content:
            soup = BeautifulSoup(html_content, "html.parser")
            table = soup.find("table")
            email_body_text = soup.get_text()  # Get plain text from the email body

            # Check if the email contains a job description
            if "jd:" in email_body_text.lower() or "job description" in email_body_text.lower():
                jd_status = "Yes"
            if table:
                ...
                df_table["Serial Number"] = serial_number
                df_table["JD"] = jd_status
                append_to_excel(df_table, TABLE_FILE_PATH)
                if "jd:" or "job description" in body1:
                    status = "Yes"
                else:
                    status = "No"
...
initialize_excel(TABLE_FILE_PATH, ["Candidate Name", "Skill", "Experience", "Current Company", "Current CTC", "Exp CTC",
                                   ...
                                   "Mobile No.", "Email id", "Job ID", "Client","Date","serial_number","JD"])
